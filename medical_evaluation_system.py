...
class BrainTumorEvaluator:
    ...
    def __init__(self, config: EvaluationConfig = None, output_dir: Optional[Path] = None):
        ...
        self.metrics_calc = MedicalSegmentationMetrics(
            num_classes=4,
            class_names=['Background', 'Core', 'Edema', 'Enhancing']
        )
    ...
    def evaluate_batch(self, predictions: np.ndarray, targets: np.ndarray,
    ...
    def _evaluate_single_case(self, prediction: np.ndarray, target: np.ndarray,
                             case_id: str, uncertainty: Optional[np.ndarray] = None) -> Dict[str, Dict[str, float]]:
    ...
    def _analyze_uncertainty(self, prediction: np.ndarray, target: np.ndarray,
    ...
    def _create_visualization(self, prediction: np.ndarray, target: np.ndarray,
                            case_id: str, uncertainty: Optional[np.ndarray] = None) -> str:
    ...
    def _apply_uncertainty_colormap(self, uncertainty: np.ndarray) -> np.ndarray:
        ...
        uncertainty_norm = ((uncertainty - uncertainty.min()) /
                          (uncertainty.max() - uncertainty.min() + 1e-8) * 255).astype(np.uint8)
    ...
    def compare_with_baseline(self, baseline_results: Dict) -> Dict[str, float]:
        ...
        comparison = {
            ...
            'relative_improvement': ((current_mean - baseline_mean) / baseline_mean * 100
                                   if baseline_mean > 0 else 0.0),
            'statistical_significance': self._statistical_test(current_dice_scores, baseline_results.get('dice_scores', []))
        }
...
class RealTimeValidator:
    ...
    def validate_epoch(self, predictions: np.ndarray, targets: np.ndarray,
                      epoch: int, case_ids: Optional[List[str]] = None) -> Dict[str, float]:

...
class BrainTumorEvaluator:
    ...
    def __init__(self, config: EvaluationConfig = None, output_dir: Path | None = None):
    ...
    def _evaluate_single_case(
        self, prediction: np.ndarray, target: np.ndarray, case_id: str, uncertainty: np.ndarray | None = None
    ...
    def get_training_summary(self) -> dict[str, float | list[float]]:
    ...
    def generate_clinical_report(self, save_path: Path | None = None) -> dict:
    ...
    def _statistical_test(self, current_scores: list[float], baseline_scores: list[float]) -> dict[str, float]:
        ...
        try:
            ...
            # T-test
            _t_stat, t_p_value = ttest_ind(current_scores, baseline_scores)

            # Mann-Whitney U test (non-parametric)
            _u_stat, u_p_value = mannwhitneyu(current_scores, baseline_scores, alternative="two-sided")
...
class RealTimeValidator:
    ...
    def validate_epoch(
        self, predictions: np.ndarray, targets: np.ndarray, epoch: int, case_ids: list[str] | None = None
    ) -> dict[str, float]:

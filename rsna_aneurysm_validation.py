#!/usr/bin/env python3
"""
RSNA Intracranial Aneurysm Detection 验证实验
验证我们的多模态脑肿瘤分割方法在动脉瘤检测任务上的泛化性
"""
...
def create_rsna_visualizations(results):
    ...
    values = [
        results['final_score'],
        results['aneurysm_present_auc'],
        np.mean(results['location_aucs'])
    ]
    ...
    # 4. 性能总结
    axes[1, 1].text(0.1, 0.8, 'RSNA Aneurysm Detection Validation', 
                   fontsize=16, fontweight='bold', transform=axes[1, 1].transAxes)
    axes[1, 1].text(0.1, 0.7, f'Final Score: {results["final_score"]:.4f}', 
                   fontsize=12, transform=axes[1, 1].transAxes)
    axes[1, 1].text(0.1, 0.6, f'Aneurysm Present AUC: {results["aneurysm_present_auc"]:.4f}', 
                   fontsize=12, transform=axes[1, 1].transAxes)
    axes[1, 1].text(0.1, 0.5, f'Location Detection AUC: {np.mean(results["location_aucs"]):.4f}', 
                   fontsize=12, transform=axes[1, 1].transAxes)
    axes[1, 1].text(0.1, 0.4, f'Cross-Institution Std: {results["cross_institution_std"]:.4f}', 
                   fontsize=12, transform=axes[1, 1].transAxes)
    axes[1, 1].text(0.1, 0.3, f'Institutions Tested: {len(results["institution_scores"])}', 
                   fontsize=12, transform=axes[1, 1].transAxes)
    ...
    plt.tight_layout()
    plt.savefig('validation_experiments/results/rsna_aneurysm/validation_results.png', 
                dpi=300, bbox_inches='tight')
    plt.savefig('validation_experiments/results/rsna_aneurysm/validation_results.pdf', 
                bbox_inches='tight')
    plt.close()
...
def analyze_rsna_advantages():
    ...
    advantages = {
        ...
        "diverse_protocols": {
            "description": "包含不同扫描仪和成像协议",
            "benefit": "验证方法的鲁棒性和泛化能力"
        },
        "expert_annotations": {
            "description": "由神经放射学专家标注",
            "benefit": "高质量标注，符合临床标准"
        },
        "large_scale": {
            "description": "大规模数据集，统计意义强",
            "benefit": "提供可靠的性能评估"
        }
    }

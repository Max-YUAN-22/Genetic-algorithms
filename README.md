...
### Brain Tumor Dataset Auto-Download (Segmentation)

```bash
yolo train model=yolo11n-seg.pt data=ultralytics/cfg/datasets/brain-tumor.yaml epochs=1 imgsz=640
```

### Fix PyTorch/Torchvision Compatibility (macOS / CPU)

If you encounter `RuntimeError: operator torchvision::nms does not exist`, install compatible CPU wheels:
...
Alternatively, on Apple Silicon:

```bash
pip install --upgrade pip
pip install 'torch==2.*' 'torchvision==0.*' --index-url https://download.pytorch.org/whl/nightly/cpu
```

Ensure torchvision version matches your torch version.

### Prepare CT+MRI Multimodal Dataset

1) Pairwise registration and fusion to CT space:

```bash
python examples/ct_mri_fusion.py \
  --ct_dir /abs/path/ct/train \
  --mri_dir /abs/path/mri/train \
  --out_root /abs/path/ct_mri_fused/train \
  --labels_dir /abs/path/labels/train \
  --fusion_method stack --resize 640 640
```

2) (Optional) Convert binary masks to YOLO polygons:

```bash
python examples/mask_to_yolo_poly.py \
  --mask_dir /abs/path/ct_mri_fused/train/labels \
  --img_dir /abs/path/ct_mri_fused/train/images \
  --out_labels /abs/path/ct_mri_fused_yolo/train/labels \
  --class_id 0
```

3) Use dataset YAML template:

```bash
cp examples/ct_mri_seg.yaml /abs/path/ct_mri_fused/ct_mri_seg.yaml
# Edit 'path:' to your absolute dataset root
yolo train model=yolo11n-seg.pt data=/abs/path/ct_mri_fused/ct_mri_seg.yaml epochs=200 imgsz=640
```

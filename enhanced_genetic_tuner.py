...
class Individual:
    ...
    def crossover(parent1: 'Individual', parent2: 'Individual') -> Tuple['Individual', 'Individual']:
        ...
        continuous_params = ['lr0', 'lrf', 'momentum', 'weight_decay', 'warmup_epochs',
                           'dice_weight', 'focal_weight', 'boundary_weight', 'aug_probability',
                           'elastic_alpha', 'noise_intensity']
...
class EnhancedGeneticTuner(Tuner):
    ...
    def __call__(self, model=None, iterations: int = None):
        ...
        for generation in range(iterations):
            ...
            LOGGER.info(f"{self.prefix}Generation {generation + 1}/{iterations}: "
            ...
            if self.config.adaptive_mutation and generation % 10 == 0:
                if len(self.convergence_history) > 10:
                    recent_improvement = (self.convergence_history[-1] -
                                        self.convergence_history[-10]) / 10

...
def create_figure1_framework():
    ...
    # Title
    ax.text(0.5, 0.95, 'Genetic Algorithm Enhanced Multimodal Brain Tumor Segmentation Framework', 
            ha='center', va='top', fontsize=16, fontweight='bold', transform=ax.transAxes)
    
    # Input data
    input_box = FancyBboxPatch((0.05, 0.75), 0.15, 0.15, 
    ...
    # Data preprocessing
    prep_box = FancyBboxPatch((0.25, 0.75), 0.15, 0.15, 
                             boxstyle="round,pad=0.02", 
    ...
    # Multimodal encoder
    encoder_box = FancyBboxPatch((0.45, 0.75), 0.15, 0.15, 
                                boxstyle="round,pad=0.02", 
    ...
    attn_box = FancyBboxPatch((0.65, 0.75), 0.15, 0.15, 
                             boxstyle="round,pad=0.02", 
    ...
    # Segmentation head
    seg_box = FancyBboxPatch((0.8, 0.75), 0.15, 0.15, 
                            boxstyle="round,pad=0.02", 
    ...
    ga_box = FancyBboxPatch((0.25, 0.5), 0.5, 0.15, 
                           boxstyle="round,pad=0.02", 
                           facecolor='lightyellow', edgecolor='gold', linewidth=2)
    ax.add_patch(ga_box)
    ax.text(0.5, 0.575, 'Genetic Algorithm Optimization', ha='center', va='center', 
            fontsize=14, fontweight='bold')
    
    # GA components
    ga_components = ['Population\nInitialization', 'Fitness\nEvaluation', 'Selection\n& Crossover', 'Mutation\n& Elitism']
    for i, comp in enumerate(ga_components):
        x = 0.3 + i * 0.1
        comp_box = FancyBboxPatch((x, 0.4), 0.08, 0.08, 
    ...
    unc_box = FancyBboxPatch((0.25, 0.2), 0.5, 0.15, 
                            boxstyle="round,pad=0.02", 
                            facecolor='lightcyan', edgecolor='teal', linewidth=2)
    ax.add_patch(unc_box)
    ax.text(0.5, 0.275, 'Uncertainty Quantification', ha='center', va='center', 
            fontsize=14, fontweight='bold')
    ...
    for i, method in enumerate(unc_methods):
        x = 0.3 + i * 0.15
        method_box = FancyBboxPatch((x, 0.1), 0.12, 0.08, 
                                   boxstyle="round,pad=0.01", 
                                   facecolor='white', edgecolor='gray', linewidth=1)
    ...
    # Output
    output_box = FancyBboxPatch((0.8, 0.2), 0.15, 0.15, 
                               boxstyle="round,pad=0.02", 
                               facecolor='lightpink', edgecolor='hotpink', linewidth=2)
    ax.add_patch(output_box)
    ax.text(0.875, 0.275, 'Segmentation\nMask + Uncertainty', ha='center', va='center', 
...
def create_figure2_architecture():
    ...
    # Title
    ax.text(0.5, 0.95, 'Multimodal Encoder-Decoder Architecture with Cross-Modal Attention', 
            ha='center', va='top', fontsize=16, fontweight='bold', transform=ax.transAxes)
    ...
    for x, y, text in ct_boxes:
        box = FancyBboxPatch((x, y), 0.12, 0.1, 
                            boxstyle="round,pad=0.01", 
                            facecolor='lightblue', edgecolor='navy', linewidth=2)
        ax.add_patch(box)
    ...
    for x, y in attn_positions:
        attn_box = FancyBboxPatch((x, y), 0.12, 0.1, 
                                 boxstyle="round,pad=0.01", 
                                 facecolor='gold', edgecolor='orange', linewidth=2)
        ax.add_patch(attn_box)
    ...
    # Segmentation head
    seg_head = FancyBboxPatch((0.9, 0.05), 0.12, 0.1, 
                             boxstyle="round,pad=0.01", 
                             facecolor='lightpink', edgecolor='hotpink', linewidth=2)
    ax.add_patch(seg_head)
    ax.text(0.96, 0.1, 'Segmentation\nHead\n(4×H×W)', ha='center', va='center', 
    ...
    for start, end in skip_connections:
        for y_offset in [0, -0.15, -0.3, -0.45]:
            ...
            if start_y >= 0.1:
                arrow = patches.FancyArrowPatch((start[0], start_y), (end[0], end_y), 
...
def create_figure4_results_comparison():
    ...
    # Dice WT
    bars1 = axes[0, 0].bar(x - width, dice_wt, width, yerr=error_wt, 
                          label='Dice WT', color='skyblue', capsize=5)
    ...
    # Dice TC
    bars2 = axes[0, 1].bar(x - width, dice_tc, width, yerr=error_tc, 
                          label='Dice TC', color='lightgreen', capsize=5)
    ...
    # Dice ET
    bars3 = axes[1, 0].bar(x - width, dice_et, width, yerr=error_et, 
                          label='Dice ET', color='lightcoral', capsize=5)
    ...
    ax2 = axes[1, 1].twinx()
    bars4 = axes[1, 1].bar(x - width/2, params, width, label='Parameters (M)', 
                          color='gold', alpha=0.7)
    bars5 = ax2.bar(x + width/2, flops, width, label='FLOPs (G)', 
...
def create_figure6_uncertainty():
    ...
    # Plot ground truth
    im1 = axes[0, 0].imshow(gt_mask, cmap='tab10')
    axes[0, 0].set_title('Ground Truth', fontweight='bold')
    axes[0, 0].axis('off')
    
    # Plot prediction
    im2 = axes[0, 1].imshow(pred_mask, cmap='tab10')
    axes[0, 1].set_title('Prediction', fontweight='bold')
    axes[0, 1].axis('off')
    
    # Plot uncertainty
    im3 = axes[0, 2].imshow(uncertainty, cmap='hot')

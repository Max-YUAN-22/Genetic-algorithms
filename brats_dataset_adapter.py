...
class BraTSDatasetLoader:
    ...
    def get_case_by_id(self, case_id: str) -> BraTSCase | None:
    ...
    def split_dataset(
        self, train_ratio: float = 0.7, val_ratio: float = 0.15, test_ratio: float = 0.15, random_seed: int = 42
        ...
        train_ids, temp_ids = train_test_split(
            case_ids, test_size=(val_ratio + test_ratio), random_state=random_seed, shuffle=True
        )

        # Second split: val vs test
        val_ids, test_ids = train_test_split(
            temp_ids, test_size=test_ratio / (val_ratio + test_ratio), random_state=random_seed, shuffle=True
        )
        ...
        self.logger.info(
            f"Dataset split: train={len(splits['train'])}, val={len(splits['val'])}, test={len(splits['test'])}"
        )
    ...
    def _apply_augmentation(
        self, primary: np.ndarray, secondary: np.ndarray, seg: np.ndarray
        ...
        if random.random() < self.config.augmentation_probability:
            ...
            primary = cv2.warpAffine(
                primary, rotation_matrix, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT
            )
            secondary = cv2.warpAffine(
                secondary, rotation_matrix, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT
            )
            seg = cv2.warpAffine(seg, rotation_matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT)
    ...
    def analyze_dataset_statistics(self) -> dict:
        ...
        stats = {
            ...
            "image_statistics": {"mean_size": None, "intensity_ranges": {}},
        }
        ...
        for case in sample_cases:
            try:
                primary, _secondary, seg = case.get_multimodal_pair()
...
def create_brats_dataloaders(
    config: BraTSConfig = None, batch_size: int = 4, num_workers: int = 2
) -> dict[str, DataLoader]:

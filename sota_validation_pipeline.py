...
class BaseSOTAModel(ABC):
    ...
    def postprocess(self, prediction: torch.Tensor, target_shape: tuple = None) -> np.ndarray:
...
class OurMultimodalGAModel(BaseSOTAModel):
    ...
    def __init__(self, checkpoint_path: Optional[str] = None, best_individual: Optional[Individual] = None):
...
class SOTAValidationPipeline:
    ...
    def _count_model_parameters(self, model: Optional[nn.Module]) -> int:
    ...
    def _generate_rankings(self) -> Dict[str, List[str]]:
        ...
        dice_ranking = sorted(
            self.results.items(),
            key=lambda x: x[1].get('mean_dice', 0.0),
            reverse=True
        )
        ...
        speed_ranking = sorted(
            self.results.items(),
            key=lambda x: x[1].get('inference_time', {}).get('mean', float('inf'))
        )
